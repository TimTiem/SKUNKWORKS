...
### Built later-adds (2026-07-17 — Tim's ask)
- **Souls-flavoured level epitaphs.** Level titles are an ascending arc of *competence* virtues (The Roused → Breaker of Inertia → … → Architect of Days → The Accomplished; overflow `Paragon N`) and each carries a one-line *epitaph* revealed at the level-up moment + shown in Stats (`domain/levels.ts` → `titleForLevel` / `epitaphForLevel`). Souls *gravitas* aimed at productivity/competence (breaking inertia → focus → execution → follow-through → mastering your time), never Souls cruelty — additive/ascendant only, no death/failure copy (P4/P8).
- **Voice control via iOS Shortcuts / Siri (headless).** Add + complete tasks hands-free. This is the app's **first server-side compute surface**: a Supabase **Edge Function** (`supabase/functions/siri`) authenticated by a **personal token** (only `sha256(token)` is stored, in the new `api_tokens` table) that resolves to a user, then calls the atomic `siri_add_task` / `siri_complete_task` RPCs (`supabase/migrations/0003_siri_api.sql`, execute-revoked from `public`). Rows land in Postgres and delta-pull into the PWA on next foreground — the local-first + append-only model is untouched. **The service-role key lives ONLY in the function (a Supabase secret), never the bundle** — the "no service-role key in the client" rule still holds. In-app token management: Settings → Voice & Siri (`features/settings/SiriSetup.tsx`). Deploy + Shortcut walkthrough: `docs/SIRI-SETUP.md`. *Not announced in-app / not version-bumped yet — it only works once the migration is run and the function is deployed.*
