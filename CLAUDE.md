...
- **Souls-flavoured level epitaphs.** Level titles are an ascending "rekindling" arc (Ashborn → Age of Fire) and each carries a one-line *epitaph* revealed at the level-up moment + shown in Stats (`domain/levels.ts` → `titleForLevel` / `epitaphForLevel`). Tone is Souls *gravitas*, never Souls cruelty — additive/ascendant only, no death/failure copy (P4/P8).
